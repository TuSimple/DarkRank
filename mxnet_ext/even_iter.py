...
class EvenIter(mx.io.DataIter):
    """
    data iterator, shuffle data but always make pairs as neighbors
    for verification and triplet loss
    """
    def __init__(self, lst_name, batch_size, aug_params=None, shuffle=False):
        super(EvenIter, self).__init__()
        self.batch_size = batch_size
        self.aug_params = aug_params.copy()
        self.shuffle = shuffle
        self.current = 0

        self.data, self.labels = EvenIter.load_data(lst_name)
    ...
    def reset(self):
        self.current = 0
        if self.shuffle:
            idx = EvenIter.even_shuffle(self.labels)
    ...
    def next(self):
        ...
        for i in range(self.current, self.current + self.batch_size):
            ...
            im = EvenIter.augment(im, self.aug_params)
            x[i - self.current] = im
